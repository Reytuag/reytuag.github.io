...
function loadFile(filename, fileIndex, callback) {
    var path = shaderFolder + filename + shaderExt;
    ...
    request.onreadystatechange = function () {
        if (request.readyState == 4) {
            if (request.status == 0 || request.status == 200) {
                callback(request.responseText, fileIndex)
            } else {
                throwError("failed to download shader: (" + path + ") " + request.statusText);
            }
        }
    };
...
function loadShaderFiles(filenames, callback) {
    var numFiles = filenames.length;
    var numComplete = 0;
    var results = [];

    function partialCallback(text, fileIndex) {
        results[fileIndex] = text;
        numComplete++;
    ...
    for (var i = 0; i < numFiles; i++) {
        loadFile(filenames[i], i, partialCallback);
    }
}
